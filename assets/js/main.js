...
let getDateAgo = (date, days) => {
  const datePre = new Date(date);

  datePre.setDate(datePre.getDate() - days);

  return datePre.toLocaleString().replace(/(\d.*),\s+(\d.*)/gu, '$1')
};
...
let car = new Car(2000, 'Tesla', 'diesel', 2020);
let car2 = new Car(5000, 'potatoes', 'infinite', 2021);
